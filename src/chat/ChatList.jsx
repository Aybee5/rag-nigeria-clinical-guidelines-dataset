  ...
  return (
    <div className={styles.tool}>
      <Icon
        ...
        type="editor"
        onClick={() => setState({ currentEditor: props.index })}
      />
      <Icon
        className={styles.icon}
        type="close"
...
export function CreateNew() {
  ...
  return (
    <div className={styles.new} onClick={newChat}>
      <Icon type="add" />
      New Conversations
    </div>
  );
          ...
          {new Array(15).fill(1).map((_, index) => (
            <div
              ...
              className={classnames(
                styles.colors_item,
                styles[`color-${index}`],
                color === index ? styles.colors_currColor : null,
              )}
            />
          ))}
        </div>
        <div className={styles.colors_box}>
          {iconList.map((item) => (
            <div
              ...
              key={item}
              style={{
                backgroundColor: ico === item && `var(--tag-color-${color})`,
              }}
              className={classnames(
                styles.colors_item,
                `ico-${item}`,
                ico === item ? styles.colors_currIco : null,
              )}
            />
          ))}
        </div>
...
export const TagIco = React.forwardRef(({ ico, color, ...rest }, ref) => (
  <div
    ref={ref}
    {...rest}
    className={classnames(styles.colors_item, `ico-${ico}`)}
    style={{ color: "#fff", backgroundColor: `var(--tag-color-${color})` }}
  />
));
  ...
  return (
    <div className={styles.editor_box}>
      ...
      <div className={styles.editor_bar}>
        <Button
          onClick={() => modifyChat({ title, icon }, props.index)}
          block={true}
          type="primary"
        >
          Save Conversations
...
export function ChatItem(props) {
  ...
  return (
    <div
      className={classnames(
        styles.item,
        currentChat === props.index && styles.current,
      )}
